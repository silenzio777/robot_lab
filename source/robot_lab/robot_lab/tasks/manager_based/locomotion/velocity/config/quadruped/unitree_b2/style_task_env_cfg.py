...
class StylePhaseCommand(CommandTerm):
    ...
    def __init__(self, cfg: "StylePhaseCommandCfg", env) -> None:
        ...
        # --- Windowed adaptive phase-matching (design train+base, 2026-09-04,
        # см. GAIT_FIDELITY_PHASE_LOCK докстринг модуля выше) ---
        # phase_time остаётся СВОБОДНЫМИ тикающими часами -- источник cos/sin
        # в observation (command property ниже), НЕ трогаем: нулевой риск для
        # bench-совместимости и уже обученных чекпоинтов формата.
        # matched_time -- ОТДЕЛЬНАЯ величина, используется ТОЛЬКО reward-
        # функциями (ref_h/ref_rp/ref_joints ниже читают её, не phase_time).
        # На каждом шаге: expected = matched_time + step_dt (клип продолжает
        # идти своим темпом, НЕ подстраивается под неподвижного робота --
        # ключевое свойство, отличающее это от "подождать пока догонит");
        # ищем среди узкого окна кандидатов вокруг expected (±0.15*duration_s)
        # тот кадр, что ближе всего к ТЕКУЩЕЙ позе робота (RAW невзвешенная
        # ошибка -- сильные суставы вроде calf дают чёткий argmin, слабые
        # thigh/hip дали бы размытый/шумный матч, если бы матчили по ним;
        # per-joint веса style_joint_pos применяются ОТДЕЛЬНО, уже после,
        # к найденному кадру -- матчинг и награда не путают роли).
        # Санити-подтверждение ДО вкрутки в обучение (train, 2026-09-04):
        # клип-сам-себя err=0/cadence=1.000x; il_slow_walk standalone
        # (kernel 0.756) err_sq~0.13-0.17/cadence~0.97x -- механизм работает.
        ...
        self._window_offsets = (
            torch.arange(-self._window_frames, self._window_frames + 1, device=self.device, dtype=torch.float32)
            * self.frame_dt
        )
        # Диагностика (НЕ reward): накопительная EMA каденса matched-часов --
        # точечный шаг-в-шаг diff шумит и систематически занижает (см.
        # memory feedback_cumulative_not_pointwise_cadence.md, тот же урок,
        # что был с zero-crossing thigh-каденсом 2026-09-03) -- EMA как
        # дешёвая накопительная замена полному окну.
    ...
    def _resample_command(self, env_ids):
        ...
        # для телепорт-сред это точный матч по построению (err=0 в момент
        # ресета, поза и есть этот самый кадр), для стоечных -- честная
        # начальная точка, окно само найдёт локально лучший матч на первых
        # шагах.
    ...
    def _update_command(self):
        ...
        # Windowed adaptive phase-matching: expected -- клип продолжает идти
        # своим темпом (НЕ подстраивается под текущее состояние робота),
        # окно ±_window_frames кадров вокруг expected ищет ближайший по RAW
        # (невзвешенной) позе кадр -- см. докстринг __init__ выше.
        step_dt = self._env.step_dt
        expected = self.matched_time + step_dt
        candidate_t = expected.unsqueeze(-1) + self._window_offsets.unsqueeze(0)  # (N, 2W+1)
        frame_idx = ((candidate_t % self.duration_s) / self.frame_dt).round().long().clamp(0, self.n_frames - 1)
        candidate_jp = self.ref_joint_pos[frame_idx]  # (N, 2W+1, n_joints)
        current_jp = self.asset.data.joint_pos[:, self.joint_ids].unsqueeze(1)  # (N, 1, n_joints)
        err = ((current_jp - candidate_jp) ** 2).sum(dim=-1)  # (N, 2W+1) -- RAW, без per-joint весов
        ...
        # границе окна (ранний сигнал, что окно "тащит", а не робот честно
        # попадает -- база предложила это как бесплатный побочный сигнал).
        inst_cadence = (step_dt + best_offset) / step_dt
        self._matched_cadence_ema = 0.98 * self._matched_cadence_ema + 0.02 * inst_cadence
        at_edge = (best_offset.abs() >= 0.9 * self._window_frames * self.frame_dt).float()
