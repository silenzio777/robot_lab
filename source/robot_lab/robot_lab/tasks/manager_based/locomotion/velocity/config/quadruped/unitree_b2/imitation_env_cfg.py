...
# 2026-09-01 (хозяин + b2): гипотеза подтвердилась -- у B2 плечевые крепления
# ног ЗНАЧИТЕЛЬНО шире, чем у лабрадора-источника mocap, а исходный IK-
# ретаргет держит следы в узкой "колее" лабрадора -- ноги вынуждены сходиться
# к центру широкого корпуса B2, отсюда и найденное вчера самопересечение
# голеней, и, вероятно, общая неустойчивость походки независимо от RSI-схемы
# (все 3 сегодняшних варианта сэмплинга упёрлись в один и тот же потолок).
# b2 подготовил варианты с раздвинутыми следами (5/10/15см от центра) --
# независимая проверка (train, MuJoCo contact-check) подтверждает
# drastic улучшение (макс. глубина проникновения упала с ~20мм до ~2-4мм),
...
# 2026-09-02 loop-ран, поводок расширен 1.3 -> 5.0 (дизайн base, данные train):
# при ДВИЖУЩЕМСЯ зацикленном референсе 2.6 м/с позиционный порог 1.3 м^2 =
# поводок 1.14 м -- любой дефицит скорости интегрируется в смерть за <1с
# (замер на it1000: POS-первым 9/20 стендовых роллаутов, смерти в
# 0.64-0.84с ровно по расчётному таймлайну; ANG/падения 11/20). Замкнутый
# круг: не выжить, пока не бегаешь 2.6, не научиться бегать -- эпизоды
# рвутся. 5.0 м^2 = поводок 2.24 м: при дефиците 1.5 м/с даёт ~1.5с жизни.
# Порог по углу НЕ трогаем -- падения должны терминировать (без поводка
# вообще вернулся бы аттрактор «стой и смотри» -- урок WALK v1). До-loop
# ран этого не видел: референс останавливался в клампе, поводок переставал
# затягиваться. ЕДИНСТВЕННОЕ изменение против loop-рана it0-1100.
DIVERGE_POS_ERR_SQ_THRESHOLD = 5.0  # m^2, ошибка root-позиции (только root, см. termination-функцию)
