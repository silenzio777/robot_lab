...
class UnitreeB2WalkResetEnvCfg(UnitreeB2RoughEnvCfg):
    ...
    def __post_init__(self):
        ...
        # ------------------------------Commands: половина сред БЕЗ heading-автопилота------------------------------
        # ФИКС YAW-ДРЕЙФА (2026-09-05, найден ХОЗЯИНОМ живьём на стенде, вид
        # строго сверху: корпус v14 развёрнут на 25-30° относительно следов при
        # прямой ходьбе). Корень: базовый velocity_env_cfg задаёт
        # heading_command=True + rel_heading_envs=1.0 -- 100% сред обучения
        # живут со встроенным P-регулятором курса (IsaacLab
        # UniformVelocityCommand._update_command: каждый шаг wz :=
        # clip(0.5*heading_error)), политика НИКОГДА не решала задачу "держи
        # прямую сам при wz=0". На стенде/железе джойстик отдаёт сырой wz=0 --
        # автопилота нет, курс никем не держится, корпус плывёт (замерено:
        # -4.7°@1с -> -40.9°@10с у v14; +22°@9.5с у walk+trot v2 -- вся линия).
        # 0.5 (дизайн base, вердикт после разбора трёх вариантов): половина
        # сред получает СЫРЫЕ константные wz-команды на эпизод (включая ~0 --
        # track_ang_vel_z_exp начинает напрямую штрафовать yaw-bias, ровно
        # недостающее давление), половина сохраняет heading-режим (плавные,
        # коррелированные с позой wz -- богатое распределение). Для obs
        # политики оба режима НЕРАЗЛИЧИМЫ (generated_commands отдаёт только
        # итоговый вектор, проверено) -- смешивание безопасно.
        ...
        # ------------------------------Rewards: base_height_l2 -- v2, ЕДИНСТВЕННОЕ изменение против v1------------------------------
        # 2026-09-02 (train+base, чистый рестарт после v1 it0-7000): v1 честно
        # проверил гипотезу "минимальный Rudin-рецепт достаточен" и опроверг её
        # для нашего B2 -- робот выучил "стоять ровно и низко" как локальный
        # оптимум (0 swing-переходов на it5200/17% бюджета, крауч углублялся
        # base_z 0.254->0.208, reward при этом честно рос до +27-36). В рецепте
        # нет высотного анкера -- "сидеть низко" ничем не наказывается, а первые
        # неуклюжие шаги дороги по штрафам. Лечение НЕ придумано: у СТАРОГО
        # WALK была ровно эта же болезнь ("приседает и ползёт", поймана хозяином
        # на стенде 2026-08-25) и вылечена именно этим термом. Сток-терм
        # унаследован ЗАНУЛЁННЫМ из rough_env_cfg.py (weight=0), target_height=
        # 0.53 там уже сконфигурирован (середина StandFix-анкера 0.50-0.56м);
        # вес -8.0 валидирован в crawl/vision конфигах. Начинаем с мягкого -8.0,
        # НЕ -16.0 (эскалация в старом WALK дала джиттер, потребовавший
        # entropy_coef=0.005) -- держим -16.0 в резерве, применять только после
        # обсуждения, если -8.0 не дожмёт. Прикидка масштаба в экономике этого
        # конфига: при крауче 0.21м штраф ~ -8.0*(0.53-0.21)^2*0.02*1000 ~=
        # -16/эпизод против ~+60 max от track_lin_vel -- ощутимое давление, не
        # доминирующее. Одна переменная за раз (урок imitation-саги 2026-09-01).
