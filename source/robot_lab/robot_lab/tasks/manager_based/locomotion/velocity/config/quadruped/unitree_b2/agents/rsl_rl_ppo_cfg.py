...
class UnitreeB2ImitationPPORunnerCfg(UnitreeB2RoughPPORunnerCfg):
    def __post_init__(self):
        ...
        # 5000 -> 15000 (2026-09-01, train+base, по факту первого/второго
        # прогона -- ровно то "скорректировать по факту", о чём предупреждал
        # старый комментарий): 5000 было явно мало -- честный per-phase sweep
        # на 3100-й итерации показал провал почти по всему клипу, не только
        # у одного сложного сегмента, значит задаче нужно кратно больше
        # бюджета, не тонкая настройка. 15000 -- тот же порядок, что
        # UnitreeB2RoughPPORunnerCfg's собственный default (20000), не
        # придумано с потолка.
